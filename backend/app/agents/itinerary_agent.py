...
from app.config import CACHE_TTL_ITINERARY, TIMEOUT_CLAUDE_ITINERARY_S, cache_key_itinerary
...
async def _generate(destination: DestinationFacts, tier: TierId, days: int) -> list[ItineraryDay]:
    user = json.dumps(
        {
...
async def run_itinerary_agent(
    *,
    ...
    if isinstance(hot, list) and len(hot) == days:
        log.info("itinerary skeleton cache hit (redis)", key=key)
    ...
    if isinstance(durable, list) and len(durable) == days:
        ...
        await cache_set(key, [d.model_dump() for d in days_models], CACHE_TTL_ITINERARY)
        return days_models, True, False

    try:
        generated = await _generate(destination, tier, days)
        ...
        if len(generated) == days:
            ...
            await cache_set(key, payload, CACHE_TTL_ITINERARY)
            await set_durable_json("itinerary_skeleton_cache", key, payload, CACHE_TTL_ITINERARY)
